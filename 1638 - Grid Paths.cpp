...
int main() {
	...
	vector< vector<int> > path_ways(n, vector<int>(n, 0));
	path_ways[0][0] = 1;
	...
	frange(n) {
		...
		for (int j = 0; j < n; j++) {
			if (row[j] == '.') {
				if (i > 0) (path_ways[i][j] += path_ways[i - 1][j]) %= mod;

				if (j > 0) (path_ways[i][j] += path_ways[i][j - 1]) %= mod;

			}	else path_ways[i][j] = 0;
		}
	}
	cout << path_ways[n - 1][n - 1] << '\n';
	return 0;
}
