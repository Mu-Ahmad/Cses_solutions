...
int main() {
	...
	vector< vector<int> > min_cost(str1.size() + 1, vector<int>(str2.size() + 1, INT_MAX));
	min_cost[0][0] = 0;
	...
	for (int i = 0; i <= str1.size(); i++) {
		for (int k = 0; k <= str2.size(); k++) {
			...
			if (i) min_cost[i][k] = min_cost[i - 1][k] + 1;

			if (k) min_cost[i][k] = min(min_cost[i][k], min_cost[i][k - 1] + 1);

			if (i and k) min_cost[i][k] = min(min_cost[i][k], min_cost[i - 1][k - 1] + (str1[i - 1] != str2[k - 1]));
		}
	}

	cout << min_cost[str1.size()][str2.size()] << '\n';
