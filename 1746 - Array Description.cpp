...
int main() {
	...
	int n, m; cin >> n >> m;
	int arr[n + 1] = {0};
	frange(n) cin >> arr[i + 1];

	//DP
	vector< vector<int> > no_of_ways(n + 2, vector<int>(m + 2, 0));
	...
	for (int i = 1; i <= n; i++) {
		for (int j = 1; j <= m; j++) {
			...
			if (i == 1) {
				...
				if (arr[i] == 0 or arr[i] == j) no_of_ways[i][j] = 1;

			} else {

				if (arr[i] == 0 or arr[i] == j)
					no_of_ways[i][j] = ((no_of_ways[i - 1][j - 1] + no_of_ways[i - 1][j]) % mod + no_of_ways[i - 1][j + 1]) % mod;
				// else
				// no_of_ways[i][j] = 0;
