...
ll solve(ll *coins, ll n, ll target, ll* min_coin) {
	//base case
	if (target == 0) return 0;

	//DP case
	if (min_coin[target]) return min_coin[target];

	//recursive case
	ll global_min = INT_MAX;
	for (ll i = 0; i < n; i++) {
		if (target >= coins[i]) {
			global_min = min(global_min, solve(coins, n, target - coins[i], min_coin));
		}
	}

	return min_coin[target] = global_min + 1;
}

int main() {
	stale_flippant();
	ll n, target; cin >> n >> target;
	ll coins[n], min_coin[target + 1] = {0};
	frange(n) cin >> coins[i];

	ll ans = solve(coins, n, target, min_coin);
	if (ans - 1 == INT_MAX) ans = -1;
	cout << ans << '\n';
	return 0;
}
