...
int main() {
	...
	w(t) {
		...
		// deb(x);
		values.insert(x);
		auto itr1 = values.lower_bound(x);
		auto itr2 = values.upper_bound(x);
		itr1--;
		// deb2(*itr1, *itr2);
		distances.insert(x - *itr1);
		distances.insert(*itr2 - x);
		distances.erase(distances.find(*itr2 - *itr1));
